...
<script type="text/javascript" src="/public/assets/bootstrap/js/bootstrap.js"></script>
    <div class="container">
        <div class="row">
            <div class="col"></div>
            <div class="col-8">
                @if(session()->has('success'))
                    ...
                    @elseif(session()->get('delete'))
                        <div class="alert alert-success">Заказ успешно удалён!</div>
                    @else
                            ...
                            <div id="order_{{$key}}" class="accordion-collapse collapse" aria-labelledby="panelsStayOpen-headingOne">
                                <div class="accordion-body">
                                    ...
                                    @if($item->status == 'Новый')
                                        <p class="small">Вы можете удалить заказ, если данный заказ является новым!</p>
                                        <a href="{{ route('order.cancel', ['order' => $item->id]) }}" class="btn btn-danger mb-1">Удалить заказ заказ</a>
